...
public class T3State {
    ...
    public Map<T3Action,T3State> getTransitions () {
        // [!] TODO
    	TreeMap<T3Action, T3State> moves = new TreeMap<>();
    	List<int[]> openTiles = getOpenTiles();
    	int[] moves1 = null;
    	for (int row = 0; row < openTiles.size(); row++) {
    		moves1 = openTiles.get(row);
    		int[] moves2 = getMoves();
    		for (int column = 0; column < moves2.length; column++) {
    			T3Action moves3 = new T3Action(moves1[0], moves1[1], moves2[column]);
    			moves.put(moves3, getNextState(moves3));
    		}
    	}
    	return moves;
    }
